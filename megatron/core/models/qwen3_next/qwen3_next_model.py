...
# [NOTE]: Refer to transformers/src/transformers/models/qwen3_next/modeling_qwen3_next.py
# 1. Support DP + (head parallel & context parallel) for linear attention layer
# 2. Support EP for MoE layer with forced balance gate; 10 in 512 experts + 1 shared one
# 3. Support DP + CP for dense attention layer
# 4. Parameter-related data management: ZeRO3
# 48 layers totally

class Qwen3NextModel(LanguageModule):
    """Qwen3-Next language model.

    Args:
        config (TransformerConfig): Model config
        qwen3_next_stack_spec (ModuleSpec): Specifies the modules to use for the various layer types
    ...
    def __init__(
        ...
        qwen3_next_stack_spec: ModuleSpec,
        vocab_size: int,
        max_sequence_length: int,
        pre_process: bool = True,
        hybrid_attention_ratio: float = 0.0,    # Useless
        hybrid_mlp_ratio: float = 0.0,  # Useless
        hybrid_override_pattern: str = None,    # Useful
        post_process: bool = True,
        fp16_lm_cross_entropy: bool = False,
        parallel_output: bool = True,
        share_embeddings_and_output_weights: bool = False,
        # Qwen3-Next with no attention has no need for position embeddings, so none is default
        ...
        self.qwen3_next_stack_spec: ModuleSpec = qwen3_next_stack_spec
        self.vocab_size = vocab_size
        self.max_sequence_length = max_sequence_length
        self.pre_process = pre_process
        self.hybrid_attention_ratio = hybrid_attention_ratio
        self.hybrid_mlp_ratio = hybrid_mlp_ratio
        assert hybrid_override_pattern is not None, '[ERROR] Only support initializing qwen3_next_stack with hybrid_override_pattern right now.'
        ...
        if self.position_embedding_type == 'rope':  # None
    ...
    def forward(
        ...
        *,
        ...
        if in_inference_mode and inference_context.materialize_only_last_token_logits:  # False, ...
        ...
        if sequence_parallel_override:  # False
            assert (
                in_inference_mode
