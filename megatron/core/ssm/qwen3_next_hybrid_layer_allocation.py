...
class Symbols:
    ...
    MLP = "-"
    MOE = 'E'
    VALID = {GDN, ATTENTION, MLP, MOE}


def _allocate_auto( # [DEPRECATED]
    total_layers_count: int, target_attention_ratio: float, target_moe_ratio: float, target_mlp_ratio: float
) -> list:
    # First, allocate attention (evenly spaced, starting and ending with qwen3_next)
    attention_layers_count: int = round(total_layers_count * target_attention_ratio)
    gdn_layers_count: int = total_layers_count - attention_layers_count
    gdn_sections_count: int = attention_layers_count + 1
    gdn_section_length: float = gdn_layers_count / gdn_sections_count

    layer_type_list = [Symbols.GDN] * total_layers_count
    x: float = gdn_section_length
    for l in range(total_layers_count):
        if x < 0.5:
            layer_type_list[l] = Symbols.ATTENTION
            x += gdn_section_length
        else:
            x -= 1
    ...
    if mlp_layers_count > 0:
        gdn_layers_count -= mlp_layers_count
        gdn_to_mlp_ratio: float = gdn_layers_count / mlp_layers_count

        x: float = gdn_to_mlp_ratio
        for l in range(total_layers_count):
            if layer_type_list[l] == Symbols.GDN:
                if x < 0.5:
...
def allocate_layers(    # DONE
    ...
    assert target_moe_ratio >= 0.0 and target_moe_ratio <= 1.0
    assert target_mlp_ratio >= 0.0 and target_mlp_ratio <= 1.0
    assert target_attention_ratio + target_moe_ratio + target_mlp_ratio <= 1.0
    # Note: target_gdn_ratio = 1.0 - target_attention_ratio - target_moe_ratio - target_mlp_ratio

    layer_type_list = _allocate_auto(total_layers_count, target_attention_ratio, target_moe_ratio, target_mlp_ratio)    # Useless, Use override_pattern !
    ...
    if target_attention_ratio > 0.0 or target_moe_ratio > 0.0 or target_mlp_ratio > 0.0 or override_pattern is not None:
        ...
        actual_moe_layers_count = layer_type_list.count(Symbols.MOE)
        actual_moe_ratio = actual_moe_layers_count / total_layers_count
        ...
        log_single_rank(
            logger,
            logging.INFO,
            f"Hybrid allocation ({Symbols.GDN} is gated_delta_net, "
        ...
        log_single_rank(
            logger,
            logging.INFO,
            f"{actual_moe_layers_count} moe layers in " f"{total_layers_count} total layers.",
        )
        log_single_rank(
            logger,
            logging.INFO,
            f"Target moe ratio: {target_moe_ratio:.2f}. "
            f"Actual moe ratio: {actual_moe_ratio:.2f}.",
        )
        log_single_rank(
            logger,
            logging.INFO,
...
def get_layer_maps_from_layer_type_list(
    ...
    for each layer type in [Attention, GDN, MLP, MoE] given a layer type list.
    """
    layer_types = [Symbols.ATTENTION, Symbols.GDN, Symbols.MLP, Symbols.MOE]
...
if __name__ == "__main__":
    test_cases = [
        ...
        # (10, 0.3, 0.2, "MM*M-*M*M-"),
        (9, 0.0, 0.0, 0.0, "D*-D*-D*-"),
        (8, 0.0, 0.0, 0.0, "DEDEDE*E"),
        (9, 0.0, 0.0, 0.0, "DDDDDDDDD"),
    ]
    for t in test_cases:
        logging.info("")
        layer_type_list = allocate_layers(*t)
        print(f'layer_type_list: {layer_type_list}', flush=True)
