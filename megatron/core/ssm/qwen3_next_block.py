...
from megatron.core.ssm.qwen3_next_hybrid_layer_allocation import Symbols as LayerSymbols
from megatron.core.ssm.qwen3_next_hybrid_layer_allocation import allocate_layers
...
@dataclass
class Qwen3NextStackSubmodules:
    """
    A class for the module specs for the Qwen3NextStack.
    """
...
class Qwen3NextStack(MegatronModule):
    """
    Constructor for the Qwen3NextStack class.

    Args:
        config (TransformerConfig): the model configuration
        submodules (Qwen3NextStackSubmodules): the submodules for the stack
    ...
    def __init__(
        ...
        submodules: Qwen3NextStackSubmodules,
        ...
        assert pg_collection is not None, "pg_collection must be provided for Qwen3NextStack"
        ...
        self.layer_type_list = allocate_layers(
            ...
            override_pattern=self.hybrid_override_pattern,
        )
    ...
    def _select_layers_for_pipeline_parallel(self, layer_type_list):
        ...
        assert self.config.virtual_pipeline_model_parallel_size is None, (
            "The Qwen3-Next hybrid model does not currently support "
    ...
    def qwen3_next_state_shapes_per_request(self) -> Optional[Tuple[Tuple[int], Tuple[int]]]:
        """
        Returns the Qwen3-Next conv and ssm states shapes per input sequence
        if this block contains Qwen3-Next layers (this may not be the case with PP > 1).
        """
        for layer_type, layer in zip(self.layer_type_list, self.layers):
            if layer_type == LayerSymbols.GDN:
                return layer.gdn_state_shapes_per_request()
        return None
    ...
    def forward(
        ...
        *,
        ...
        """
        Forward function of the Qwen3NextStack class.
    ...
    def sharded_state_dict(
        ...
        for local_layer_idx, layer in enumerate(self.layers):
            ...
            state_dict_prefix = (
                f'{layer_prefix}{local_layer_idx}.'  # module list index in Qwen3NextBlock
            )
